...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    responseHandler: $.common.isEmpty(options.responseHandler)?$.table.responseHandler:options.responseHandler,           // 在加载服务器发送来的数据之前处理函数
                    onLoadSuccess: $.common.isEmpty(options.onLoadSuccess)?$.table.onLoadSuccess:options.onLoadSuccess,               // 当所有数据被加载时触发处理函数
			...
			initEvent: function() {
				...
				$(optionsIds).on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table load-success.bs.table", function () {
					...
					// 非多个禁用
                    $('#' + toolbar + ' .multiple').toggleClass('disabled', rows.length==0);
					// 非单个禁用
    		...
    		exportExcel: function(formId) {
				...
				$.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
					...
					$.post(table.options.exportUrl, $("#" + currentId).serializeArray(), function(result) {
						console.log($("#" + currentId).serializeArray())
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
					if (dict.dictValue == ('' + value)) {
                    	var listClass = $.common.equals(dict.listClass) || $.common.isEmpty(dict.listClass) ? "" : "badge badge-" + dict.listClass;
        ...
        operate: {
            ...
            removeAll: function() {
				table.set();
				var rows = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);        		if (rows.length == 0) {
					if (rows.length == 0) {
						$.modal.alertWarning("请至少选择一条记录");
						return;
					}
			...
			addTray: function(id) {
				table.set();
				$.modal.open("添加" + table.options.modalName, $.operate.addTrayUrl(id));            },
            // 添加信息，以tab页展现
            addTab: function (id) {
				table.set();
			...
			addTrayUrl: function(id) {
				var url = $.common.isEmpty(id) ? table.options.createTrayUrl.replace("{id}", "") : table.options.createUrl.replace("{id}", id);
                return url;
            },
            // 修改信息
            edit: function(id) {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.wms_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
